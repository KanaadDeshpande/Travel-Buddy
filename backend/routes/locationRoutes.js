  ...
  try {
    if (admin(req)) {
      const location = new Location({
        ...
        locationType: args.locationInput.location.type,
        locationCoordinates: args.locationInput.location.coordinates,
        weatherNote: args.locationInput.weather.note,
        weatherTemperature: args.locationInput.weather.temperature,
  ...
  try {
    ...
    if (locations) {
      ...
      const locations = await Location.find({
        category: args.locationId,
      }).populate('category');

      if (locations) {
        redis.setex(
          'category:locations:' + args.locationId,
  ...
  try {
    if (admin(req)) {
      ...
      const newLocation = await Location({
        name: args.updateLocation.location,
      });
